...
export class Goodie extends Role {
  ...
  constructor(cfg: { onCollect?: (g: Actor, h: Actor) => boolean } = {}) {
    ...
    let goodieSound = game.musicLibrary.getSound("foodPickup.wav");
    // Provide a default onCollect handler
    this.onCollect = (cfg.onCollect) ? cfg.onCollect : (_g: Actor, _h: Actor) => { game.score.goodieCount[0]++; goodieSound.play(); return true;};
  }
...
export class Obstacle extends Role {
  ...
  public playCollideSound() {
    this.collideSound = game.musicLibrary.getSound("landSound.wav");
    //collideSound = "step3.mp3";
